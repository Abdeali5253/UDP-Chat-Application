            ...
            if client_address != sender_address:
                if file_path:
                    ...
                    with open(file_path, 'rb') as f:
                        ...
                        header = f'{os.path.basename(file_path)}:{filesize}'.encode(
                        )
                        self.server_socket.sendto(header, client_address)
                        self.server_socket.sendto(file_data, client_address)
                    self.message_listbox.insert(
                        tk.END, f'{message_with_username} (file sent)')
                else:
                    # Send text message
                    self.server_socket.sendto(
                        message_with_username.encode(), client_address)
